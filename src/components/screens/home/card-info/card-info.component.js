...
import { BALANCE_UPDATE } from '@/constansts/event.constants'
import { formatCardNumber } from '@/utilites/format/format-card-number'
...
export class CardInfo extends ChildComponent {
	...
	#addListener() {
		document.addEventListener(BALANCE_UPDATE, this.#onUpdatedBalance)
	}

	#removeListener() {
		document.removeEventListener(BALANCE_UPDATE, this.#onUpdatedBalance)
	}
